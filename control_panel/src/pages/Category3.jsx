...
const Category3 = () => {
  ...
  const fetchCategories = async () => {
    try {
      const { data } = await axios.get("/categories3");
      setCategories(data);
    } catch (error) {
  ...
  const handleUpload = async (e) => {
    ...
    setLoading(true);
    try {
      await axios.post("/categories3", formData, {
  ...
  const handleUpdate = async (e) => {
    ...
    setLoading(true);
    try {
      await axios.put(`/categories3/${editCategoryId}`, formData, {
        headers: { "Content-Type": "multipart/form-data" },
      });
  ...
  const handleDelete = async (id) => {
    if (window.confirm("Are you sure you want to delete this category?")) {
      try {
        await axios.delete(`/categories3/${id}`);
        fetchCategories();
      } catch (error) {
  ...
  return (
    <div className="p-6">
      <h1 className="text-3xl font-bold mb-6">Manage MEP & HVAC Work</h1>
